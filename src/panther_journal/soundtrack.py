...
def schema(db):
    n.schema(db)
    db.execute(
        "CREATE TABLE IF NOT EXISTS soundtrack_projects (id TEXT PRIMARY KEY, content TEXT NOT NULL)"
    )
    db.execute(
        "CREATE TABLE IF NOT EXISTS soundtrack_plans (id TEXT PRIMARY KEY, content TEXT NOT NULL, approved INTEGER NOT NULL DEFAULT 0)"
    )
    db.execute(
        "CREATE TABLE IF NOT EXISTS soundtrack_attempts (id TEXT PRIMARY KEY, plan_id TEXT NOT NULL UNIQUE, project TEXT NOT NULL, reserved_cents INTEGER NOT NULL CHECK(reserved_cents>0), state TEXT NOT NULL, content TEXT NOT NULL)"
    )
...
def history(db, project):
    ...
    for table, reader in [("attempts", v.read_plan), ("narration_attempts", n.read_plan)]:
        for row in db.execute(f"SELECT * FROM {table}"):
            ...
            endpoint = (
                plan["endpoint"]
                if table == "narration_attempts"
                else plan["inputs"][row["shot"]]["endpoint"]
            )
            records.append(
                dict(
                    table=table,
...
def reconcile(project, maximum, reason, fal):
    ...
    with v.database() as db:
        schema(db)
        existing = db.execute(
            "SELECT content FROM soundtrack_projects WHERE id=?", (project,)
        ).fetchone()
        if existing:
            prior_audit = json.loads(existing["content"])
            if (
                prior_audit["soundtrackLimitCents"] != maximum
                or prior_audit["ownerAuthorization"] != reason
            ):
                v.fail(
                    "Reconciliation already exists with different approval facts; no reset supported."
                )
    ...
    audit = dict(
        ...
        historicalBilledUsd=str(amount),
        soundtrackLimitCents=maximum,
        history=prior,
        billingEvents=events,
        ownerAuthorization=reason,
...
def status(db, project):
    ...
    if (
        b["schemaVersion"] != 1
        or not b["ownerAuthorization"]
        or history(db, project) != b["history"]
        or b["capCents"] != original["capCents"]
        or b["billingAccount"] != original["billingAccount"]
    ...
    if (
        total != v.number(b["historicalBilledUsd"])
        or v.cents(total) + b["soundtrackLimitCents"] > b["capCents"]
    ):
        v.fail("Invalid reconciled allowance.")
    held = db.execute(
        "SELECT COALESCE(SUM(reserved_cents),0) FROM soundtrack_attempts WHERE project=?",
        (project,),
    ).fetchone()[0]
    return {
        **b,
        "soundtrackReservedCents": held,
        "availableCents": b["soundtrackLimitCents"] - held,
    }
...
def validate(m):
    ...
    if any(
        not re.fullmatch(
            r"games/" + re.escape(m["gameId"]) + r"/assets/[a-z0-9-]+/original/[^/?\\]+", k
        )
        or ".." in k
...
def payload(m):
    ...
    if m["kind"] == "music":
        return dict(
            prompt=m["text"],
            music_length_ms=m["seconds"] * 1000,
            force_instrumental=True,
            output_format="mp3_44100_128",
        )
    return dict(
        text=m["text"],
        duration_seconds=m["seconds"],
...
def quote(m, fal):
    ...
    prices = fal.request(
        "GET", v.PLATFORM + "/models/pricing", params={"endpoint_id": endpoint}
    ).get("prices", [])
    matching = [p for p in prices if p.get("endpoint_id") == endpoint]
    if (
        len(matching) != 1
        or matching[0].get("currency") != "USD"
        or matching[0].get("unit") != unit
    ):
    ...
    units = (
        Decimal(math.ceil(m["seconds"] / 60))
        if m["kind"] == "music"
        else Decimal(m["seconds"])
        if m["kind"] == "effects"
        else Decimal(len(m["text"].encode())) / 1000
    )
    return {
        "endpoint": endpoint,
        "unit": unit,
...
def read_plan(db, pid):
    ...
    if (
        hashlib.sha256(v.canonical(p).encode()).hexdigest() != pid
        or payload(m) != p["input"]
        or PROFILES[m["kind"]][0] != p["quote"]["endpoint"]
    ):
...
def prepare(m, fal):
    ...
    with v.database() as db:
        ...
        if (
            b["gameId"] != m["gameId"]
            or b["billingAccount"] != p["billingAccount"]
            or q["reserveCents"] > b["availableCents"]
        ):
            v.fail("Project scope or soundtrack budget mismatch.")
        db.execute(
            "INSERT OR IGNORE INTO soundtrack_plans (id,content) VALUES (?,?)",
            (pid, v.canonical(p)),
        )
...
def update(aid, state, extra):
    with v.database() as db:
        ...
        db.execute(
            "UPDATE soundtrack_attempts SET state=?,content=? WHERE id=?",
            (state, v.canonical(d), aid),
        )
        return attempt(
            db.execute("SELECT * FROM soundtrack_attempts WHERE id=?", (aid,)).fetchone()
        )
...
def submit(pid, fal):
    ...
    with v.database() as db:
        ...
        if not approved:
            v.fail("Explicit exact-plan audio/voice/text/budget approval required.")
    m = p["manifest"]
    q = quote(m, fal)
    billing = fal.billing()
...
def poll(aid, fal):
    with v.database() as db:
        row = db.execute("SELECT * FROM soundtrack_attempts WHERE id=?", (aid,)).fetchone()
        if not row:
            v.fail("Unknown soundtrack attempt.")
        if row["state"] in ("COMPLETED", "FAILED"):
            return attempt(row)
        d = json.loads(row["content"])
        p, _ = read_plan(db, row["plan_id"])
    if not d.get("urls"):
        v.fail("Unknown submission; inspect before proceeding.")
    ep = p["quote"]["endpoint"]
    rid = d["requestId"]
    ...
    if state.get("status") in ("IN_QUEUE", "IN_PROGRESS"):
        return update(aid, state["status"], {})
    if state.get("status") != "COMPLETED":
        v.fail("Unknown provider state.")
    try:
        result = fal.request(
            "GET", v.queue_url(d["urls"]["response"], rid, ep, "response"), completed_result=True
        )
    except v.TerminalModelRejection:
        return update(aid, "FAILED", {"reason": "Provider rejection; no retries."})
    if not isinstance(result.get("audio"), dict):
        v.fail("Missing audio; reservation retained.")
...
def download(aid, fal):
    with v.database() as db:
        ...
        if not row or row["state"] != "COMPLETED":
            v.fail("Completed audio required.")
        d = json.loads(row["content"])
        p, _ = read_plan(db, row["plan_id"])
    folder = v.private_root() / "soundtrack-outputs"
    ...
    if not target.exists():
        ...
        if temp.exists():
            v.fail("Partial download exists; inspect before recovery.")
        with requests.Session() as session:
            ...
            with session.get(
                ...
                if response.status_code != 200:
                    v.fail("Download failed; do not regenerate.")
                size = 0
                with temp.open("xb") as out:
                    ...
                    for block in response.iter_content(1024 * 1024):
                        size += len(block)
                        if size > 32 * 1024**2:
                            v.fail("Audio exceeds 32 MiB bound.")
                        out.write(block)
                    out.flush()
                    os.fsync(out.fileno())
    ...
    if (
        not d.get("sha256")
        and hashlib.sha256((folder / (aid + ".partial")).read_bytes()).hexdigest() != sha
    ):
        v.fail("Original conflicts with downloaded evidence.")
    if d.get("sha256") and d["sha256"] != sha:
        v.fail("Original bytes changed.")
    ...
    meta = dict(
        ...
        extra=dict(
            ...
            contextUse="exclude",
            generation=g.fal(p["quote"]["endpoint"], d["requestId"], billed),
        ),
    )
    if m["voice"]:
        meta["extra"]["voice"] = m["voice"]
    ...
    if not provenance.exists():
        write_json(
            provenance, dict(plan=p, sha256=sha, requestId=d["requestId"], billingEvent=event)
        )
    return update(
        aid, "COMPLETED", dict(downloadPath=str(target), sha256=sha, metadataPath=str(mp))
    )
...
def reconcile_command(project, maximum, reason, owner_approved):
    if not owner_approved:
        v.fail("Owner approval required.")
...
def approve_command(plan_id, owner_approved, auto_topup_disabled):
    if not owner_approved or not auto_topup_disabled:
        v.fail("Explicit rights, casting, text/model sharing and spending approval required.")
    with v.database() as db:
        p, _ = read_plan(db, plan_id)
        if p["quote"]["reserveCents"] > status(db, p["manifest"]["projectId"])["availableCents"]:
            v.fail("Allowance exceeded.")
...
def command(name, func):
    ...
    def run(identity):
        ...
        click.echo(
            json.dumps(
                {k: val for k, val in result.items() if k not in ("urls", "result")}, indent=2
            )
        )
