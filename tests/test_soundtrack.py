...
class Fal:
    ...
    def billing(self):
        return dict(account=self.account, balanceUsd="50")

    def billing_events(self, ids):
        return {k: self.events[k] for k in ids if k in self.events}

    def request(self, method, url, **kw):
        if method == "GET" and url.endswith("/pricing"):
            ep = kw["params"]["endpoint_id"]
            profile = next(p for p in s.PROFILES.values() if p[0] == ep)
            return {
                "prices": [
                    dict(
                        endpoint_id=ep,
                        currency="USD",
                        unit=profile[1],
                        unit_price=str(self.rate or profile[2]),
                    )
                ]
            }
        self.posts += 1
...
def setup(tmp_path, monkeypatch):
    ...
    with v.database(initialize=True):
        pass
    fal = Fal()
    n.create_budget("film", "game", "15", "13", "Owner approved", fal)
    m = dict(
        schemaVersion=1,
        projectId="film",
        gameId="game",
        purpose="audition",
        text="A fictional line",
        voice="Brian",
        direction="Dramatic",
        sourceKeys=["games/game/assets/script/original/script.json"],
    )
    p = n.prepare(m, fal)
    with v.database() as db:
        db.execute(
            ...
            (
                ...
                1000,
                "COMPLETED",
                json.dumps({"requestId": "old-request"}),
            ),
        )
...
def manifest(kind="music"):
    return dict(
        ...
        kind=kind,
        title="Synthetic cue",
        text="Original dramatic instrumental score",
        seconds=240 if kind == "music" else 8,
        sourceKeys=["games/game/assets/script/original/script.json"],
    )


def opened(f):
    return s.reconcile("film", 400, "Owner approved soundtrack and reconciliation", f)


def approved(f, m=None):
    p = s.prepare(m or manifest(), f)
    with v.database() as db:
        db.execute("UPDATE soundtrack_plans SET approved=1 WHERE id=?", (p["planId"],))
    return p
...
def test_reconciliation_preserves_history_and_closes_old_writers(setup):
    ...
    with v.database() as db:
        ...
        with pytest.raises(click.ClickException):
            v.assert_generation_open(db, "film")
...
def test_bad_evidence_blocks_reconciliation(setup, case):
    if case == "missing":
        setup.events = {}
    if case == "endpoint":
        setup.events["old-request"]["endpoint"] = "wrong"
    if case == "negative":
        setup.events["old-request"]["amount"] = "-1"
    if case == "account":
        setup.account = "different"
    if case == "unknown":
        with v.database() as db:
            db.execute("UPDATE narration_attempts SET state='UNKNOWN'")
    with pytest.raises(click.ClickException):
        opened(setup)
...
def test_cap_and_history_conflicts(setup):
    with pytest.raises(click.ClickException):
        s.reconcile("film", 600, "Owner approved", setup)
    opened(setup)
    with v.database() as db:
...
def test_price_increase_and_unapproved(setup):
    opened(setup)
    p = s.prepare(manifest(), setup)
    with pytest.raises(click.ClickException):
        s.submit(p["planId"], setup)
    p = approved(setup)
    setup.rate = Decimal("1")
    with pytest.raises(click.ClickException):
        s.submit(p["planId"], setup)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("kind", "unbounded"),
        ("seconds", 241),
        ("sourceKeys", ["games/other/assets/script/original/script.json"]),
        ("extra", "unknown"),
    ],
)
